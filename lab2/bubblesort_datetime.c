...
int compare(struct Date d1,struct Date d2)
{
    ...
    {return 1;}
    if(d1.yy==d2.yy && d1.mm==d2.mm && d1.dd==d2.dd && d1.hr<d2.hr)
    {return 1;}
    if(d1.yy==d2.yy && d1.mm==d2.mm && d1.dd==d2.dd && d1.hr==d2.hr && d1.min<d2.min)
    {return 1;}
    if(d1.yy==d2.yy && d1.mm==d2.mm && d1.dd==d2.dd && d1.hr==d2.hr && d1.min==d2.min && d1.sec<d2.sec)
    {return 1;}
...
int main()
{
    ...
    for (int i = 0; i < n; i++)
    {
        fprintf(fp2,"%d-%d-%d %d:%d:%d\n",arr[i].dd,arr[i].mm,arr[i].yy,arr[i].hr,arr[i].min,arr[i].sec);
    }
    end2=clock();
    fclose(fp2);
